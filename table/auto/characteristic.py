...
def parse(output= Output(host='MySQL-8.2', user='root', password='', database='example')):
    ...
    for i in range(characteristic.count()):
        ...
        data = {
            'sub_category_id': 15,
            'name': f'{atr['name']} автомобиля',
            'options': json.dumps(options, ensure_ascii=False),
            'input': '{'+f'"type": "select", "label": "выберете {atr["name"]} автомобиля","placeholder": "не выбрано"'+'}',
            'filter': '{'+f'"type": "checkboxGroup", "label": "выберете {atr["name"]} автомобиля","placeholder": "не выбрано"'+'}',
            'isFilter': 1,
        }
        output.insert([data])
        log_debug(f'успешно создан атрибут {atr['name']} автомобиля с {len(options)} вариантами значений')
